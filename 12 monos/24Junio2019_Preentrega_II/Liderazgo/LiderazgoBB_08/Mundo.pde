class Mundo
{
...

  {
    ...
    sumaX=40;// es el valor máximo del tam de bolitaReactiva
    sumaY=-sumaX/2;
    comodin=0;
    ...
    for (int i=0; i <cantBoli; i++)
    {
      ...
      bandaUP[i].setTam(50);
      bandaUP[i].setVel(2*cantBoli-i*1.5);

      bandaDOWN[i]= new BolitaReactiva();
      bandaDOWN[i].setTam(45);
      bandaDOWN[i].setVel(2*cantBoli-i*1.5);
      //  bandaDOWN[i].setNum(random);

      lSeguidores[0][i]= new Lider();
      lSeguidores[1][i]= new Lider();

      //  BolitaReactiva(float vel_, int tam_,float umbral_,float alpha_)
      bSeguidoresUP[i]= new BolitaReactiva(2*cantBoli-i*1.5, 5, sumaX+(i*sumaX), 100);
      bSeguidoresDOWN[i]= new BolitaReactiva(2*cantBoli-i*1.5, 5, sumaX+(i*sumaX), 100);
    }
  }
